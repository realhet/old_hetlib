...
type
   ...
   TPerlin3DNoise = class (TObject)
      ...
      public
         ...
         function Noise(const v : TV3f) : Single; overload;
         function Noise(const v : TV4f) : Single; overload;
  end;
...
procedure SinCos(const a,r:single;out s,c:single);
begin
  s:=sin(a)*r;
  c:=cos(a)*r;
end;
...
begin
   ...
   for i:=0 to cPERLIN_TABLE_SIZE-1 do begin
      z:=1-2*Random;
      r:=Sqrt(1-z*z);
      SinCos(pi*2*Random, r, FGradients[i*3], FGradients[i*3+1]);
      FGradients[i*3+2]:=z;
   end;
...
function TPerlin3DNoise.Noise(const v : Tv3f) : Single;
...
begin
   Result:=Noise(v3f(x, y, z));
end;

// Noise (hmg)
//
function TPerlin3DNoise.Noise(const v : TV4f) : Single;
type PV3f=^TV3f;
begin
   Result:=Noise(PV3f(@v)^);
end;
